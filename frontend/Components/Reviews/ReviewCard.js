    ...
    deleteReview(review_id)
      .then(() => {
        ...
        const average_rating = (
          (averageRating * reviewCount - rating_for_location) /
          (reviewCount - 1)
        ).toFixed(1);
  ...
  return (
    <View style={styles.reviewCard}>
      ...
      <View style={styles.starrating}>
        <StarRating rating={rating_for_location !== null ? rating_for_location : 0} />
      </View>
      <Text style={styles.reviewBody}>{body}</Text>
      <View style={styles.timeandvotes}>
        <Text style={styles.timeAgo}>
          <Icon name="clock-with-white-face" color="black" />
        </Text>
      ...
      {isDeleted && (
        <Text style={styles.deleteText}>
          Review Deleted - review will disappear from list in a few seconds
        </Text>
      )}
    </View>
  );
